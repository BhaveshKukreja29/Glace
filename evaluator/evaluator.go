...
// --- GLACE-003: BreakSignal is used to break out of loops ---
// BreakSignal is used to break out of loops.
type BreakSignal struct{}

func (b *BreakSignal) Error() string { return "break signal" }

// --- GLACE-003: ContinueSignal is used to skip to next loop iteration ---
...
func Eval(node ast.Node, env *Environment) (Value, error) {
	...
	// --- GLACE-003: Handle break statement ---
	case *ast.BreakStatement:
		return nil, &BreakSignal{}
	// --- GLACE-003: Handle continue statement ---
...
func evalLoopStatement(stmt *ast.LoopStatement, env *Environment) (Value, error) {
	...
	for {
		...
		if err != nil {
			// --- GLACE-003: Catch break signal and exit loop ---
			if _, ok := err.(*BreakSignal); ok {
				break
			}
			// --- GLACE-003: Catch continue signal and skip to next iteration ---
...
func evalForInLoop(stmt *ast.LoopStatement, iterable Value, env *Environment) (Value, error) {
	...
	case *ArrayValue:
		for _, elem := range iter.Elements {
			...
			_, err := Eval(stmt.Body, loopEnv)
			if err != nil {
				// --- GLACE-003: Catch break signal in for-in loops ---
				if _, ok := err.(*BreakSignal); ok {
					break
				}
				// --- GLACE-003: Catch continue signal in for-in loops ---
				if _, ok := err.(*ContinueSignal); ok {
					continue
				}
				return nil, err
	...
	case *RangeValue:
		for i := iter.Start; i < iter.End; i += iter.Step {
			...
			_, err := Eval(stmt.Body, loopEnv)
			if err != nil {
				// --- GLACE-003: Catch break signal in range loops ---
				if _, ok := err.(*BreakSignal); ok {
					break
				}
				// --- GLACE-003: Catch continue signal in range loops ---
				if _, ok := err.(*ContinueSignal); ok {
					continue
				}
				return nil, err
